...
document.addEventListener('DOMContentLoaded', () => {
    ...
    console.log('🔍 Checking for all card types...');
    ...
    // Fallback: Show cards immediately if animations fail
    setTimeout(() => {
        // Handle all card types
        [...solutionCards, ...contentCards, ...productCards].forEach(card => {
            if (!card.classList.contains('visible')) {
                console.log('⚠️ Adding visible class manually to card:', card.className);
        ...
        staggerContainers.forEach(container => {
            if (!container.classList.contains('visible')) {
                console.log('⚠️ Adding visible class to stagger container');
                container.classList.add('visible');
            }
        });
    }, 1000);
    
    // Special handling for cards that already have 'visible' class
    const visibleCards = document.querySelectorAll('.content-card.visible, .product-card.visible');
    ...
    const visibleStaggerContainers = document.querySelectorAll('.stagger-animation');
